...
async def exercise(base_url: str, token: str) -> dict[str, Any]:
    async with httpx2.AsyncClient(headers={"Authorization": f"Bearer {token}"}) as http:
        transport = streamable_http_client(f"{base_url.rstrip('/')}/mcp", http_client=http)
...
def main() -> None:
    ...
    token = os.environ.get("FOLIO_API_TOKEN", "")
    if not token:
        raise RuntimeError("FOLIO_API_TOKEN is required")
    print(json.dumps(asyncio.run(exercise(base_url, token)), sort_keys=True))
