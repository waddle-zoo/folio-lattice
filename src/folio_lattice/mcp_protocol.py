...
TOOLS: list[dict[str, Any]] = [
    {
        ...
        "description": "Create an artifact and its first immutable version.",
        "inputSchema": {
            "type": "object",
    ...
    {
        "name": "artifact_write",
        "description": "Write a new immutable version with an optimistic parent check.",
        "inputSchema": {
            "type": "object",
            "required": ["artifact_id", "content_base64"],
            "properties": {
                "tenant_id": {"type": "string"},
    ...
    {
        "name": "artifact_read",
        "description": "Read a complete artifact version.",
        "inputSchema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Read one bounded document chunk.",
        "inputSchema": {
            "type": "object",
            "required": ["chunk_id"],
    ...
    },
    {
        ...
        "description": "Search indexed text with SQLite FTS5.",
        "inputSchema": {
            "type": "object",
            "required": ["query"],
            "properties": {
    ...
    {
        "name": "artifact_grep",
        "description": "Run a bounded regular-expression search over indexed chunks.",
        "inputSchema": {
            "type": "object",
            "required": ["pattern"],
            "properties": {
    ...
    {
        "name": "graph_link",
        "description": "Create or update a typed edge between artifacts.",
        "inputSchema": {
            "type": "object",
            "required": ["source_artifact_id", "target_artifact_id", "edge_type"],
            "properties": {
                "tenant_id": {"type": "string"},
    ...
    {
        "name": "graph_traverse",
        "description": "Traverse outgoing graph edges with explicit bounds.",
        "inputSchema": {
            "type": "object",
            "required": ["start_artifact_id"],
            "properties": {
                "tenant_id": {"type": "string"},
                "start_artifact_id": {"type": "string"},
    ...
    def _call(self, name: str, args: dict[str, Any]) -> Any:
        ...
        if name == "artifact_create":
            ...
            return self.service.create_artifact(
                ...
                actor=str(args.get("actor") or "dev"),
                reason=str(args.get("reason") or "initial artifact"),
        ...
        if name == "artifact_write":
            ...
            return self.service.write_version(
                ...
                data=base64.b64decode(args["content_base64"], validate=True),
                media_type=str(args.get("media_type") or artifact["media_type"]),
                actor=str(args.get("actor") or "dev"),
                reason=str(args.get("reason") or "update"),
                source_context=args.get("source_context") or {},
                parent_version_id=args.get("parent_version_id"),
            )
        if name == "artifact_read":
            return self.service.read_artifact(
                tenant_id, str(args["artifact_id"]), args.get("version_id")
            )
        ...
        if name == "graph_link":
            return self.service.link(
                ...
                str(args["target_artifact_id"]),
                str(args["edge_type"]),
                args.get("metadata") or {},
            )
        if name == "graph_traverse":
            return self.service.traverse(
                tenant_id,
                str(args["start_artifact_id"]),
                int(args.get("max_depth") or 2),
                int(args.get("limit") or 100),
            )
        if name == "artifact_versions":
            return self.service.versions(
                tenant_id, str(args["artifact_id"]), int(args.get("limit") or 100)
            )
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "folio-lattice", "version": "0.1.0"},
                },
            }
