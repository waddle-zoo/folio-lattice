...
async function loadAudit(id, {preserveState = false} = {}) { const arguments_ = {limit: 128}; if (id) arguments_.connection_id = id; try { renderAudit(await callConnection('external_mcp_audit', arguments_)); } catch (error) { renderAudit([]); if (!preserveState) adminError(error); } }
